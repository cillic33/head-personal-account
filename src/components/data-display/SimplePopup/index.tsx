import {IProps, IStyledOverlayProps} from "./props";
      ...
      {isPopupShow &&
        <StyledOverlay ref={overlayRef} onClick={handleCloseClick}>`
          <StyledPopup>
            ...
            <Flexbox $padding="16px 24px" $gap="16px" $background="#fff" $align="center" $justify="flex-end" $borderRadius="0 0 12px 12px">
              <Button $variant="primary" size="m" onClick={handleCancelClick}>{cancelTitle}</Button>
              <Button $variant="dark" size="m" onClick={handleSubmitClick}>{submitTitle}</Button>
            </Flexbox>
          </StyledPopup>
        </StyledOverlay>
      }
